  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(
      title: const Row(
        ...
        children: [Icon(Icons.eco, size: 24), SizedBox(width: 8), Text('匠心农场')],
      ),
      centerTitle: true,
      elevation: 0,
      actions: [
...
class _HumidityCard extends StatelessWidget {
  ...
  String _buildSensorSummary(bool hasData) {
    ...
    if (!deviceState.isSensorOk) {
      return '传感器数据异常';
    }

    final humidity = deviceState.humidity;
    if (humidity < deviceState.thL) {
      return '土壤偏干';
    }
    if (humidity < deviceState.thH) {
      return '土壤正在回湿';
    }
    if (humidity < deviceState.thH + 10) {
      return '土壤湿度适中';
    }
    return '土壤较湿润';
  }
                ...
                Row(
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 13,
                        color: isPumpOn ? Colors.green : Colors.grey[600],
                      ),
                    ),
                  ],
                ),
            ...
            if (deviceState.isLocked) ...[
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  onPressed: !isConnected || isOperating
                      ? null
                      : () => onUnlock(),
